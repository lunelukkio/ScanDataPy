...
class TsmFileIo:
    ...
    def read_data(self):
        try:
            ...
            full_dark_framesize = tuple(
                self.full_3D_size + [0, 0, 1])  # including dark frame
            ...
            self.full_frames = self.full_frames - self.dark_frame[:, :,
                                                  np.newaxis]

            self.ch_frames = self.split_frames(self.full_frames,
                                               self.num_fluo_ch)
    ...
    def get_infor(self) -> list:
        ...
        return ([
                    self.full_frame_interval]
                + [self.ch_frame_interval]
                * self.num_fluo_ch + [elec_interval]
                * self.elec_data_obj.num_elec_ch
                )

    def get_3d(self) -> tuple:
        return self.full_frames, self.ch_frames[:, :, :, 0], self.ch_frames[:,
                                                             :, :, 1]
...
class DaFileIo:
    ...
    def read_data(self):
        try:
            with open(self.full_filename, 'rb') as f:
                ...
                self.header = np.frombuffer(binary_data[:5120],
                                            dtype=np.int16)  # header is first 5120 bytes (2560 integers)
                read_data_byte = self.header.nbytes
                # str_header = self.header.decode()

                # x pixel.  97th Integer-6408 80*80+8?
                num_x = int(
                    self.header[384])  # This should be int, not np.int16
                # y pixel
                ...
                pre_frames = np.frombuffer(
                    binary_data[
                    read_data_byte:pixel_count * 2 + read_data_byte
                    ], dtype=np.int16
                )
                ...
                self.elec_interval = self.full_frame_interval / self.bnc_ratio
                self.num_elec_data = self.num_full_frames * self.bnc_ratio
                self.elec_trace = np.empty(
                    [self.num_elec_data, 8])  # = [data, ch]
                ...
                self.full_frames = pre_full_frames - self.dark_frame[:, :,
                                                     np.newaxis]
    ...
    def get_infor(self) -> list:
        return ([self.full_frame_interval]
                + [self.ch_frame_interval]
                * self.num_fluo_ch
                + [self.elec_interval]
                * 8
