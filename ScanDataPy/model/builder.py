...
class DaBuilder:
    def __init__(self, filename_obj):
        ...
        ch_list = [f"CH{i}" for i in
                   range(num_ch + 1)]  # This includes full trace
        ...
        self.data_interval_dict = dict(
            zip(infor_keys, data_interval))  # make an interval dict
        ...
        del file_io  # release the io object to allow file changes during recording.
        print(
            "----- DaBulder: The .da file was imported and the file_io object was deleted.")
        print("")
    ...
    def get_frame(self) -> dict:  # change to numpy to value obj
        data = {"CH" + str(i): FramesData(self.frames[i],
                                          self.data_interval_dict[
                                              "CH" + str(i) + "_INTERVAL"]) for
                i in range(num_ch + 1)}
