...
class KeyManager:
    ...
    def get_lists_of_tag_list(self) -> list:
        ...
        all_lists = [
            self.filename_list,
            self.attribute_list,  # ['Data']
            self.data_type_list,  # ['FluoFramesCh1', 'FluoFramesCh2']
            self.origin_list  # ['File']
        ]

        # internal function
        def recursive_combinations(current_combination, remaining_list):
            if not remaining_list:  # if there is no remaining_dicts,
            ...
            for tag in current_list:
                ...
                new_combination.append(tag)
                # delete the current remaining object
                recursive_combinations(new_combination, remaining_list[1:])
