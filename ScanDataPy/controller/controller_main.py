...
from ScanDataPy.view.list_window import DataListWindow
from ScanDataPy.controller.controller_data import DataController
from ScanDataPy.controller.controller_filename import FileService, WholeFilename, FileHistoryManager
...
class MainController:
    def __init__(self, gui_app=None):
        ...
        if self.gui_backend_name == 'pyqt6':
            ...
            if sys.flags.interactive == 0 and self.scandata: # Check if scandata is not None
                self.scandata.exec()
            ...
            print(f"[MainController:ERROR] Unsupported GUI backend: {self.gui_backend_name}")
            print("[MainController] Application will be terminated.")
    ...
    def open_file(self):
        ...
        try:
            ...
            print(f"[MainController] Main file list updated with files sharing extension with {filename_obj.name}.")
        except Exception as e:
            print(f"[MainController:ERROR] Failed to update main list with same-extension files for {filename_obj.name}: {e}")
    ...
    def _prepare_data_controller(self, filename_obj: WholeFilename) -> tuple[DataController | None, bool]:
        """
        ...
        """
        if not filename_obj: 
            print("[MainController:_prepare_data_controller] Received no valid filename_obj. Cannot prepare DataController.")
            return None, False 
            
        assert filename_obj is not None, "[MainController:ERROR] _prepare_data_controller received a null filename_obj despite initial check. This indicates a logic error."

        if filename_obj.name in self.data_controller_dict:
            print(f"[MainController] DataController for {filename_obj.name} already exists. Using existing instance.")
            return self.data_controller_dict[filename_obj.name], False
        
        try:
            print(f"[MainController] Creating new DataController for {filename_obj.name}.")
            data_controller = DataController(filename_obj, self.gui_backend_name)
            self.data_controller_dict[filename_obj.name] = data_controller
            print(f"[MainController] New DataController for {filename_obj.name} created and stored.")
            self.history_manager.add_file(filename_obj)
            return data_controller, True
        except Exception as e:
            print(f"[MainController:ERROR] Error creating DataController for {filename_obj.name}: {e}")
            if filename_obj.name in self.data_controller_dict: # Clean up if partially added
    ...
    def display_file_history(self):
        ...
        if not recent_files:
            ...
            # Optionally, show a message box to the user via the main_list_window if it exists
            if hasattr(self, 'main_list_window') and self.main_list_window.isVisible():
                QtWidgets.QMessageBox.information(self.main_list_window, "File History", "File history is currently empty.")
            return
        ...
        print("-------------------------")
        # Here you could implement a dialog to show these files and allow opening them.
        # For example, using QtWidgets.QInputDialog.getItem or a custom dialog.
        raise NotImplementedError('Histry function is under construction.')
