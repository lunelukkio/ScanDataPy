...
from SCANDATA.common_class import KeyManager
...
class AxesController(metaclass=ABCMeta):
    ...
    def set_mod_key_list(self, mod_key):
        ...
        print(f"AxesController: Current mod set[{self.__class__.__name__}]: {self._mod_key_list}")
...
class TraceAxesController(AxesController):
        ...
        for true_dict in true_dict_list:
            ...
            if self.mode == "CH_MODE":
                for key in true_dict.values():
                    if 'Elec' in key:
                        plot_data.setPen(pg.mkPen(color=self._ch_colors['Elec0']))
                        return
                    for color in self._ch_colors:
                        if key in color:
                            if color:
                                plot_data.setPen(pg.mkPen(color=self._ch_colors[key]))
                    ...
                    for color in self._controller_colors:
                        if key in color:
                            if color:
                                ...
                                plot_data.setPen(pg.mkPen(color=self._controller_colors[key]))
    ...
    def set_marker(self):
        ...
        for controller_dict in true_controller_list:
            ...
            roi_controller_list = [key for key in controller_dict.values() if "ROI" in key]
        for controller_key in roi_controller_list:
            # get roi value
            controller_list = self._model.get_controller({'Attribute':'UserController', 'ControllerName':controller_key})
            ...
            if controller_key in self._marker_obj:
                ...
                self._marker_obj[controller_key] = ROIBox(self._controller_colors[controller_key])
                self._marker_obj[controller_key].set_roi(box_pos)
